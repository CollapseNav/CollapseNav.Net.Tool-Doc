...
由于原生的DI不支持 `属性注入`, 所以打算尝试实现一下

## 思考1

正常来说, 我们希望的 `属性注入` 使用起来应该像下面的代码一样
...
首先参考装饰器模式扩展出一个 `MyServiceProvider`
...
```
:::tip
在获取属性之前最好使用 `GetType()` 获取到的 `type`

`serviceType` 很有可能只是接口的类型, 而不是具体的实现类型
:::
...
原生DI使用一个 `ServiceProviderFactory` 创建 `ServiceProvider` 对象

所以在自定义了 `MyServiceProvider` 之后, 我们还需要自定义一个 `MyServiceProviderFactory`
...
这主要是因为 `Controller` 由 `ControllerActivator` 创建
...
有了自己的 `MyControllerActivator` 之后, 就需要将原来注册的 `IControllerActivator` 替换掉
